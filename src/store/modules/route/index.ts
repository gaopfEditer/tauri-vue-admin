...
export const useRouteStore = defineStore('route-store', {
  ...
  actions: {
    ...
    async initDynamicRoute(): Promise<boolean> {
      ...
      const { data, error } = await fetchUserRoutes(userId);
      if (!data?.routes?.length) {
        window.$message?.error(
          error?.msg || '权限路由加载失败：请确认 MySQL 已连接且 Tauri 终端出现 desktop api server listening'
        );
        return false;
      }
    ...
    async initAuthRoute() {
      ...
      if (isDynamicRoute) {
        routeReady = await this.initDynamicRoute();
      } else {
